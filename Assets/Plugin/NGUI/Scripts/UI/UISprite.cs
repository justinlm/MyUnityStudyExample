...
public class UISprite : UIBasicSprite
{
    ...
    public void MakePixelPerfect(float scale) {
        ...
        if (mType == Type.Simple || mType == Type.Filled || !sp.hasBorder)
        {
            if (tex != null)
            {
                int x = Mathf.RoundToInt(pixelSize * (sp.width + sp.paddingLeft + sp.paddingRight)*scale);
                int y = Mathf.RoundToInt(pixelSize * (sp.height + sp.paddingTop + sp.paddingBottom)*scale);
