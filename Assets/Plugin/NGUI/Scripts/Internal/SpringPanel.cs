...
/// <summary>
/// Similar to SpringPosition, 
/// but also moves the panel's clipping. Works in local coordinates.
/// </summary>
...
public class SpringPanel : MonoBehaviour
{
    ...
    static public SpringPanel Begin(GameObject go, Vector3 pos, float strength,OnFinished action)
    {
        SpringPanel sp = go.GetComponent<SpringPanel>();
        if (sp == null) sp = go.AddComponent<SpringPanel>();
        sp.target = pos;
        sp.strength = strength;
        sp.onFinished = action;
        sp.enabled = true;
        return sp;
    }
}
